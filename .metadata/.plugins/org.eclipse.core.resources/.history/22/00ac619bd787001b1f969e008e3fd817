...
void vTask2_cmd_handling(void *params)
{
	...
	while(1)
	{
		...
		//to serialise the access of vTask2_cmd_handling and interrupt handler
		//if not, accessing the global_Command buffer can cause risc condition possibility
		//so temporarly disable interrupts with the taskENTER_CRITICAL()
		taskENTER_CRITICAL();
...
{
	APP_CMD_t *new_cmd;
	//the task_msg is used here just to use these 50 char shared by all the tasks that needs a print and not use more space than needed
	char task_msg[50];

	uint32_t toggle_duration = pdMS_TO_TICKS(1000);
...
//		}
		...
		//more efficient than a if els if structure in case of constant values and equality comparison
		...
		//function variables are allocated every time the function is called, so -->
...
//the xTimer is passed as parmeters because the xTimerCreate function (in led_toggle_start) requires it
void led_toggle(TimerHandle_t xTimer)
{
